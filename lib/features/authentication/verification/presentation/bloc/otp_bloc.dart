...
class OTPBloc extends Bloc<OTPEvent, OTPState> {
  ...
  Future<void> _onCheckCodeEvent(
      CheckCodeEvent event, Emitter<OTPState> emit) async {
    emit(LoadingOTPState());
    print(
        "\ud83d\udce4 OTP Verification Request Sent: \${event.checkCodeEntity}");
    ...
    failureOrSuccess.fold(
      ...
      (success) async {
        ...
        if (success.token != null && success.token!.isNotEmpty) {
          ...
          print(
              "\u26a0\ufe0f Warning: No token received after OTP verification.");
        }
  ...
  Future<void> _onResendCodeEvent(
      ResendCodeEvent event, Emitter<OTPState> emit) async {
    emit(LoadingOTPState());
    print(
        "\ud83d\udce4 Resending OTP for Email: \${event.resendCodeEntity.email}");
